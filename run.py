...
def inference(
    ...
    pipe.set_progress_bar_config(desc="DRF inference")
...
def main(args):
    ...
    """
    for SDXL hotshotco : 512x512 SDXL
    """
    ...
    structure.save(path.join(args.output_folder, f"structure.jpg"), quality=JPEG_QUALITY)
    appearance.save(path.join(args.output_folder, f"appearance.jpg"), quality=JPEG_QUALITY)
    result.save(path.join(args.output_folder, f"result.jpg"), quality=JPEG_QUALITY)
    if result_refiner is not None:
        result_refiner.save(path.join(args.output_folder, f"result_refiner.jpg"), quality=JPEG_QUALITY)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", "-o", type=str, default="./results/DRF")
