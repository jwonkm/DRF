...
def get_last_control_i(control_schedule, num_inference_steps):
    ...
    for block in control_schedule.values():
        ...
        for layer in block.values():
            structure_max = max(structure_max, max_(layer[0] + layer[1])) # [0],[1]이 structure쪽 정보
            appearance_max = max(appearance_max, max_(layer[2])) # [2]가 appearance쪽 정보
...
class DRFPipeline(StableDiffusionXLPipeline):  # diffusers==0.28.0
    ...
    def prepare_latents(
        ...
        image = image.to(device=device, dtype=dtype) # tensor일 경우
        ...
        # noise tensor, init_latents tensor output
    ...
    """
    (1)preprocessing -> (2)text encoding -> (3)structure/appearance latents -> (4)schedule/timestep -> (5)Denoising loop -> (6)Image decoding
    """
    @torch.no_grad()
    def __call__(
        self,
        ...
        **kwargs,
        ...
        #! Dual Recursive Feedback Loss
        self.dual_loss = DualLoss(t_min=50, t_max=950, unet=self.unet, scale=1.0, scheduler=self.scheduler, iter_fp=3, device=self.device)
        ...
        # multi prompt, 여러 image 생성 (batch) 기능은 지원 X
        ...
        # structure prompt가 없으면 일반 prompt embeds 공유
        else:
        ...
        # todo: pipeline edit part
        # 5. Prepare latent variables
        num_channels_latents = self.unet.config.in_channels

        # image=None -> general latent noise
        ...
        # structure image -> latents
        if structure_image is not None:
            structure_image = preprocess( 
        ...
        else: # appearance image X -> general latents
        ...
        # denoising_end, denoising_start -> timestep change
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for _ in range(self_recurrence_schedule[i]):
                    ...
                    # latent for dual_feedback
                    latents_drf = latents
                    latents, eps = noise_t2t(self.scheduler, t_prev, t, latents)

                    register_attr(self, t=t.item(), do_control=False, batch_order=["uncond", "cond"])
                    added_cond_kwargs_app = {"text_embeds": appearance_add_text_embeds, "time_ids": add_time_ids}
                    added_cond_kwargs = {"text_embeds": add_text_embeds, "time_ids": add_time_ids}

                    latents_app, _, latents_gen, _ = self.dual_loss.update_drf(
                        clean_appearance_latents, 
                        latents_drf,
                        t.unsqueeze(0),
                        t_prev,
                        eps,
                        appearance_prompt_embeds.unsqueeze(0),
                        prompt_embeds.unsqueeze(0),
                        timestep_cond = timestep_cond,
                        cross_attention_kwargs = self.cross_attention_kwargs,
                        added_cond_kwargs_1 = added_cond_kwargs_app,
                        added_cond_kwargs_2 = added_cond_kwargs,
                    )
                    latents = latents_gen
        ...
        return DRFPipelineOutput(images=image, structures=structure, appearances=appearance)
